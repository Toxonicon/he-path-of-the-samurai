...
// API для ISS данных (Rate Limit: 120 req/min, Redis cache DISABLED - install php-redis extension)
Route::prefix('api/iss')->middleware([ApiRateLimiter::class . ':120,1'])->group(function() {
...
// API для OSDR данных (Rate Limit: 60 req/min, Redis cache DISABLED)
Route::prefix('api/osdr')->middleware([ApiRateLimiter::class . ':60,1'])->group(function() {
...
// JWST галерея (Rate Limit: 30 req/min, Redis cache DISABLED)
Route::get('/api/jwst/feed', [DashboardController::class, 'jwstFeed'])
    ->middleware([ApiRateLimiter::class . ':30,1']);

// Астрономические события (Rate Limit: 60 req/min, Redis cache DISABLED)
Route::middleware([ApiRateLimiter::class . ':60,1'])->group(function() {
    Route::get('/api/astro/events', [AstroController::class, 'events']);
    Route::get('/api/astronomy-events', [AstroController::class, 'events']);
});

// Pascal Legacy telemetry (Rate Limit: 60 req/min, Redis cache DISABLED)
...
Route::get('/api/legacy', [LegacyController::class, 'api'])
    ->middleware([ApiRateLimiter::class . ':60,1']);
